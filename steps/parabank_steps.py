...
def step_impl(context):
    context.parabank_register_page.verify_error()

#Verify login function

@when('I insert the username "{username}"')
def step_impl(context, username):
    context.parabank_register_page.insert_username_login(username)

@when('I insert the password "{password}"')
def step_impl(context, password):
    context.parabank_register_page.insert_password_login(password)

@when('I click Log in button')
def step_impl(context):
    context.parabank_register_page.login_button()

@then('It should appear a message "Welcome" and the Accounts Overview table')
def step_impl(context):
    context.parabank_register_page.welcome_message()

#Open new account

@when('I click Open New Account')
def step_impl(context):
    context.parabank_register_page.open_new_acc()

@when('I select the type of Account I want to open')
def step_impl(context):
    context.parabank_register_page.select_type_of_account()

@when('I select an existing account to transfer funds into the new account')
def step_impl(context):
    context.parabank_register_page.select_account()
    time.sleep(1)

@when('I click Open New Account button')
def step_impl(context):
    context.parabank_register_page.open_account_button()

@then('It appears the new account number')
def step_impl(context):
    context.parabank_register_page.open_account_number()

#Verify new account is created

@when('I click Accounts Overview')
def step_impl(context):
    context.parabank_register_page.account_overview_click()

@then('It should appear the new account ID opened in the table')
def step_impl(context):
    context.parabank_register_page.new_account_verified()

#Bill Pay

@when('I click Bill Pay from the menu')
def step_impl(context):
    context.parabank_register_page.bill_pay_button()
...
def step_impl(context, payee_name, address, city, state, zip_code, phone_number, account_number, verify_account_number):
    context.parabank_register_page.insert_payee_details(payee_name, address, city, state, zip_code, phone_number, account_number, verify_account_number)

@when('I type the ammount of money')
def step_impl(context):
    context.parabank_register_page.pay_money()

@when('I select the account')
def step_impl(context):
    context.parabank_register_page.select_payee_account()

@when('I select Send Payment button')
def step_impl(context):
    context.parabank_register_page.send_payment_button()

@then('It appears Bill Payment Complete')
def step_impl(context):
    context.parabank_register_page.verify_bill_payment_complete()

#Verify new balance

@then('Then It should appear the account used with less amount of money because of the bill')
def step_impl(context):
    context.parabank_register_page.verify_new_balance()
...
def step_impl(context):
    context.parabank_register_page.click_update_contact_info()

@when('I change the last name with "{new_last_name}"')
def step_impl(context, new_last_name):
    context.parabank_register_page.update_last_name(new_last_name)

@when('I change the phone number with "{new_phone_number}"')
def step_impl(context, new_phone_number):
    context.parabank_register_page.update_phone_number(new_phone_number)

@when("I click Update profile")
def step_impl(context):
    context.parabank_register_page.click_update_profile()

@then('I receive errors about fields required to change')
def step_impl(context):
    context.parabank_register_page.profile_update_message()

#clear_database
@when('I click on Admin Page')
def step_impl(context):
    context.parabank_register_page.admin_page_click()

@when('I click on Clean')
def step_impl(context):
    context.parabank_register_page.clean_database_button()

@when('It appears a message that Database Cleaned')
def step_impl(context):
    context.parabank_register_page.database_cleaned_message()


@then('It will appear "The username and password could not be verified." message that no user was found')
def step_impl(context):
    context.parabank_register_page.no_user_found()
